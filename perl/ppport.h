...
#define PERL_BCDVERSION \
    ((PERL_REVISION * 0x1000000L) + (PERL_VERSION * 0x1000L) + PERL_SUBVERSION)
...
#define newSVpvn(data, len) \
    ((data) ? ((len) ? newSVpv((data), (len)) : newSVpv("", 0)) : newSV(0))
#endif
...
#define dMY_CXT_SV  \
    SV *my_cxt_sv = \
	*hv_fetch(PL_modglobal, MY_CXT_KEY, sizeof(MY_CXT_KEY) - 1, TRUE)
#endif /* < perl5.004_68 */
...
#define SvPV_nolen(sv) \
    ((SvFLAGS(sv) & (SVf_POK)) == SVf_POK ? SvPVX(sv) : sv_2pv_nolen(sv))
    ...
    int DPPP_(my_grok_number)(pTHX_ const char *pv, STRLEN len, UV *valuep)
{
	...
	if (GROK_NUMERIC_RADIX(&s, send)) {
	    ...
	    while (s < send &&
		   isDIGIT(*s)) /* optional digits after the radix */
		s++;
	}
...
static UV DPPP_(my_grok_bin)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			     NV *result);
static
#else
extern UV DPPP_(my_grok_bin)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			     NV *result);
#endif
...
#define Perl_grok_bin	     DPPP_(my_grok_bin)
    ...
    UV DPPP_(my_grok_bin)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			  NV *result)
{
    const char *s = start;
    STRLEN len	  = *len_p;
...
static UV DPPP_(my_grok_hex)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			     NV *result);
static
#else
extern UV DPPP_(my_grok_hex)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			     NV *result);
#endif
...
#define Perl_grok_hex	     DPPP_(my_grok_hex)
    ...
    UV DPPP_(my_grok_hex)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			  NV *result)
{
    const char *s = start;
    STRLEN len	  = *len_p;
...
static UV DPPP_(my_grok_oct)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			     NV *result);
static
#else
extern UV DPPP_(my_grok_oct)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			     NV *result);
#endif
...
#define Perl_grok_oct	     DPPP_(my_grok_oct)
    ...
    UV DPPP_(my_grok_oct)(pTHX_ char *start, STRLEN *len_p, I32 *flags,
			  NV *result)
{
    const char *s = start;
    STRLEN len	  = *len_p;
